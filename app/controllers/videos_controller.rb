class VideosController < ApplicationController
  ...
  private
    ...
    def process_search_params
      ...
      if params[:tag]
        @search = Video.search(params[:q]).where('is_active =?',true)
        @videos = @search.result(:distinct => true).tagged_with(params[:tag]).where('is_active =?',true).order("created_at desc").page(params[:page]).per(10)
      
      else
        @search=Video.search(params[:q])
        @videos= @search.result(:distinct => true).last_x_days(time_days).where('is_active =?',true).order("created_at desc").page(params[:page]).per(10)
        if params[:q]
