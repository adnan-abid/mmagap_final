class LessonCheckoutsController < ApplicationController
  ...
  def create
    ...
    if !@a.nil?
      ...
      #@lesson_checkout = LessonCheckout.new(params[:LessonCheckout])
      #@lesson_checkout.save
      #@orderid = @lesson_checkout.id;
      
      @la = LessonCheckout.connection.insert("INSERT INTO lesson_checkouts (user_id,video_id, payment_status,created_at, updated_at) VALUES (#{@vid},#{@uid},'pending','#{@dd}','#{@dd}')")
